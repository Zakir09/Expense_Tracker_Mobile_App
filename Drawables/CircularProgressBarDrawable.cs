...
namespace ExpenseTracker.Drawables
{
	...
	{
		public static readonly BindableProperty BudgetProperty = BindableProperty.Create(nameof(Budget), typeof(int), typeof(CircularProgressBarDrawable));
		...
		public static readonly BindableProperty BudgetColorProperty = BindableProperty.Create(nameof(BudgetColor), typeof(Color), typeof(CircularProgressBarDrawable));
		...
		public int Budget
		{
			get => (int)GetValue(BudgetProperty);
		...
		public Color BudgetColor
		{
			get { return (Color)GetValue(BudgetColorProperty); }
			set { SetValue(BudgetColorProperty, value); }
		}
		...
		public void Draw(ICanvas canvas, RectF dirtyRect)
		{
			...
			if (Budget < 0) Budget = 0; else if (Budget > 100) Budget = 100;
			...
			{ 
				...
				// Draw arc budget
				float budgetangle = GetAngle(Budget);
				canvas.StrokeColor = BudgetColor;
				canvas.StrokeSize = Thickness;
				canvas.DrawArc(x, y, effectiveSize, effectiveSize, 90, budgetangle, true, false);
